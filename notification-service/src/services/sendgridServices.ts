...
const sendWelcomeFoundingListEmailService =
    ...
    async (
      ...
      const messageDetails = {
        ...
        trackingSettings: {
          ...
          asm: {
            group_id: parseInt(
              process.env.SENDGRID_UNSUBSCRIBE_GROUP_ID || "0"
            ),
            groups_to_display: [
      ...
      try {
        const emailResponse = await sendgridDetails.sendgridMail.send(
          messageDetails
        );
...
const addToSendGridFoundersList = errorUtilities.withServiceErrorHandling(
  async (
    email: string,
    firstName: string,
    lastName: string,
...
const rollbackDeleteOperations = async (
  ...
  try {
    ...
    if (rollbackResponse[0].statusCode === 202) {
      console.log(
        "Rollback successful: User re-added to",
        completedListIds.length,
        "lists"
      );
    } else {
      console.error(
        "Rollback failed with status:",
        rollbackResponse[0].statusCode
      );
    }
...
const removeFromSengridFoundersListService =
  errorUtilities.withServiceErrorHandling(async (email: string) => {
    ...
    try {
      ...
      const userFirstName =
        findContact?.body?.result[email]?.contact?.first_name;
      const userLastName = findContact?.body?.result[email]?.contact?.last_name;
      ...
      for (const operation of deleteOperations) {
        try {
          ...
          if (
            deleteResponse[0].statusCode !== 202 &&
            deleteResponse[0].statusCode !== 204
          ) {
            throw new Error(
              `Delete failed for ${operation.name}: Status ${deleteResponse[0].statusCode}`
            );
          }
          ...
          console.error(
            `Error deleting from ${operation.name}:`,
            operationError
          );

          await rollbackDeleteOperations(
            email,
            userFirstName,
            userLastName,
            userCountry,
      ...
      if (addResponse[0].statusCode !== 202) {
        ...
        throw errorUtilities.createError(
          "Failed to add to unsubscribe list. All operations rolled back.",
          addResponse[0].statusCode
        );
      }
      ...
      console.error("Error Unsubscribing:", error.response?.body || error);
      throw errorUtilities.createError(
        `Error Unsubscribing: ${error.response?.body || error}`,
        500
      );
    }
  });

const addToSendGridBetaTestersListService =
  errorUtilities.withServiceErrorHandling(
    async (userDetails: Record<string, any>) => {
      ...
      try {
        ...
        console.error(
          `Error adding contact ${firstName} to update list:`,
          error.response?.body?.errors[0]?.message && error.response?.body
        );
        throw errorUtilities.createError(
...
const addToSendGridContributorsListService =
  errorUtilities.withServiceErrorHandling(
    async (userDetails: Record<string, any>) => {
      const { email, firstName, lastName, country } = userDetails;
      ...
      try {
        ...
        console.error(
          `Error adding contact ${firstName} to contributors list:`,
          error.response?.body?.errors[0]?.message && error.response?.body
        );
        throw errorUtilities.createError(
          `${error.response?.body?.errors[0]?.message}`,
...
const addToSendGridUpdatesListService = errorUtilities.withServiceErrorHandling(
  async (userDetails: Record<string, any>) => {
    const { email, firstName, lastName, country } = userDetails;
...
const addCustomFieldToSendgridList = errorUtilities.withServiceErrorHandling(
  async (fieldData: Record<string, any>) => {
    ...
    if (!listId) {
      throw errorUtilities.createError(
        `Missing list ID for list: ${listName}`,
        500
      );
    }
    ...
    try {
      ...
      console.error(
        `1 Error adding field to ${listName} list:`,
        error.response?.body?.errors[0]?.message
      );
      throw errorUtilities.createError(
        `${error.response?.body?.errors[0]?.message}`,
...
const sendgridSendPasswordResetLinkService =
  errorUtilities.withServiceErrorHandling(
...
const sendgridSendPasswordResetConfirmationService =
  errorUtilities.withServiceErrorHandling(
...
const sendNotificationChangeService = errorUtilities.withServiceErrorHandling(
  async (email: string, firstName: string, notificationPreference:string) => {
    const html = renderEmailTemplate("notification-preference.html", {
      firstName,
...
const sendPaymentFailedEmailService = errorUtilities.withServiceErrorHandling(
  async (
    email: string,
    firstName: string,
    planName: string,
    ...
    const html = renderEmailTemplate("payment-failed.html", {
      firstName,
      planName,
      amount,
      LOGO_URL:
