...
export const comparePronounciation = async (req: Request, res: Response) => {
  if (!req.file) {
    return res.status(400).json({ message: "File required" });
  }

  const userfileName = req.file.filename.replace(/\.[^/.]+$/, "");
  const userRawFilePath = path.join(
    __dirname,
    "../audioFiles/uploads/raw",
    req.file.filename
  );
  const userWavFilePath = path.join(
    __dirname,
    "../audioFiles/uploads/wavs",
    `${userfileName}.wav`
  );
  const userTrimmedFilePath = path.join(
    __dirname,
    "../audioFiles/uploads/trimmed",
    `${userfileName}.wav`
  );
  const normalisedUserWavPath = path.join(
    __dirname,
    "../audioFiles/uploads/normalized",
    `${userfileName}.wav`
  );

  const masterFileName = "labalaba_master.wav";
  const masterRawFilePath = path.join(
    __dirname,
    "../audioFiles/master/raw",
    masterFileName
  );
  const masterWavFilePath = path.join(
    __dirname,
    "../audioFiles/master/wavs",
    masterFileName
  );
  const masterTrimmedFilePath = path.join(
    __dirname,
    "../audioFiles/master/trimmed",
    masterFileName
  );
  const normalisedMasterWavPath = path.join(
    __dirname,
    "../audioFiles/master/normalized",
    "labalaba_master.wav"
  );
  ...
  try {
    ...
    if (isInvalid(userRawAudio)) {
      throw Error("Your recording was too short or too quiet");
    }
    if (isInvalid(masterRawAudio)) {
      throw Error("Reference audio is too short or too quiet");
    }
    ...
    res.status(200).json({
      ...
      finalScore,
      dtwPlot: plotDTWResult.secure_url,
      plot: plotResult.secure_url,
    });
  } catch (error) {
    fsPromises.unlink(userRawFilePath);
    fsPromises.unlink(userTrimmedFilePath);
