...
const addPhrase = errorUtilities.withErrorHandling(
  async (phrasePayload: Record<string, any>): Promise<Record<string, any>> => {
    let { yoruba_text, english_text, pronounciation_note, phrase_category } =
      phrasePayload;

    const existingEnglishPhrase =
      (await phraseRepository.phrasesRepository.getOne({
        english_text,
      })) as unknown as PhraseAttributes;

    const existingYorubaPhrase =
      (await phraseRepository.phrasesRepository.getOne({
        yoruba_text,
      })) as unknown as PhraseAttributes;

    if (existingEnglishPhrase) {
      throw errorUtilities.createError("English phrase already exists", 400);
    }

    if (existingYorubaPhrase) {
      throw errorUtilities.createError("Yoruba phrase already exists", 400);
    }
    ...
    if (!newPhrase) {
      throw errorUtilities.createError("Unable to create phrase", 400);
    }
...
const addManyPhrases = errorUtilities.withErrorHandling(
  async (phraseArray: Record<string, any>[]): Promise<Record<string, any>> => {
    ...
    for (const phrasePayload of phraseArray) {
      ...
      const existingEnglishPhrase =
        await phraseRepository.phrasesRepository.getOne({ english_text });
      const existingYorubaPhrase =
        await phraseRepository.phrasesRepository.getOne({ yoruba_text });
      ...
      const newPhrase: Record<string, any> =
        await phraseRepository.phrasesRepository.create(phraseCreationPayload);
    ...
    if (createdPhrases.length === 0) {
      throw errorUtilities.createError(
        "No new phrases were created because all the phrases already exist",
        400
      );
    }
...
const findOrphanedFilesService = errorUtilities.withErrorHandling(
  async (
    ...
    return cloudinaryFiles.filter((cloudFile) => {
      ...
      if (
        databaseFiles.includes(cloudFile.url) ||
  ...
  async (): Promise<Record<string, any>> => {
    ...
    return audioRecords.map((record) => {
      const plainRecord: any =
        record instanceof Recordings ? record.get({ plain: true }) : record;
...
const getPhraseWithAllRecordingsForZabbot = errorUtilities.withErrorHandling(
  async (englishText: string, yorubaText: string): Promise<any> => {
    if (!englishText && !yorubaText) {
      throw errorUtilities.createError(
        "Either English Text or Yoruba Text must be provided",
        400
      );
    }
    ...
    const checkPhrase: Partial<PhraseAttributes> | any =
      await phraseRepository.phrasesRepository.getOne(filter, [
        "id",
        "yoruba_text",
        "english_text",
        "pronounciation_note",
    ...
    if (!checkPhrase) {
      throw errorUtilities.createError(
        "This phrase does not have a recording in our database",
        404
      );
    }

    const findRecordings =
      await recordingRepository.recordingRepository.getMany({
...
const getPhrasesWithAllRecordingsForZabbotParallel =
  errorUtilities.withErrorHandling(
    async (
      phrases: Array<{ englishText?: string; yorubaText?: string }>
    ): Promise<any> => {
      if (!phrases || !Array.isArray(phrases) || phrases.length === 0) {
        throw errorUtilities.createError(
          "Phrases array must be provided and cannot be empty",
          400
        );
      }
      ...
      const processPhrasePromises = phrases.map(async (phrase) => {
        ...
        try {
          ...
          return {
            error:
              error.message || "An error occurred while processing this phrase",
            englishText: englishText || null,
            yorubaText: yorubaText || null,
            recordings: [],
          };
...
const getAllRecordings = errorUtilities.withErrorHandling(
  async (page: number = 1, search?: string): Promise<any> => {
    const limit = 10;
    const offset = (page - 1) * limit;

    const recordings = await recordingRepository.recordingRepository.getMany(
      {}
    );
    ...
    const phraseIds = recordings
      .map((rec: any) => rec.phrase_id)
      .filter(Boolean);

    const phrases = await phraseRepository.phrasesRepository.getMany({
      id: phraseIds,
    });

    const phraseMap = new Map(
      phrases.map((phrase: any) => [phrase.id, phrase])
    );

    let formattedResponse = recordings.map((recording: any) => {
    ...
    if (search) {
      const normalizedSearch = generalHelpers.normalizeYoruba(search);

      formattedResponse = formattedResponse.filter((item: any) => {
        const normalizedYoruba = generalHelpers.normalizeYoruba(
          item.yorubaText
        );

        return (
          normalizedYoruba.includes(normalizedSearch) ||
