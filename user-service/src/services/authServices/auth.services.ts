...
const registerUserService = errorUtilities.withServiceErrorHandling(async (registerPayload: UserAttributes) => {
    ...
    return responseUtilities.handleServicesResponse(StatusCodes.Created, role && role === UserRoles.ADMIN ? GeneralResponses.ADMIN_REGISTRATION_SUCCESSFUL : GeneralResponses.USER_REGSTRATION_SUCCESSFUL, createUserPayload.email)
});


const verifyUserAccountService = errorUtilities.withServiceErrorHandling(async (email: string, otp: string) => {

    const user = await userRepositories.getOne({ email }, ['id', 'email', 'role', 'isVerified']);
    ...
    const otpData: OtpAttributes | any = await otpRepositories.getLatestOtp(
        { userId: user.id, isUsed: false, notificationType: OtpNotificationType.EMAIL },
        ['id', 'otp', 'expiresAt', 'isUsed', 'attempts']
    );

    if (!otpData) {
        throw errorUtilities.createError(OtpResponses.INVALID_OTP, StatusCodes.NotFound);
    }
    if (otpData.attempts >= 4) {
        throw errorUtilities.createError(OtpResponses.OTP_EXCEEDED_ATTEMPTS, StatusCodes.Forbidden);
    }
    ...
    if (!isOtpValid) {
        ...
        throw errorUtilities.createError(OtpResponses.INVALID_OTP, StatusCodes.Unauthorized);
    }
    ...
    if (otpData.expiresAt < new Date()) {
        ...
        throw errorUtilities.createError(OtpResponses.OTP_EXPIRED, StatusCodes.Unauthorized);
    }
...
const resendVerificationOtpService = errorUtilities.withServiceErrorHandling(async (email: string) => {
    const user = await usersRepositories.getOne(
        { email: email },
        ['id', 'email', 'role', 'firstName', 'isVerified']
    );
    ...
    if (!otpCreated) {
        console.error('ERROR====> OTP CREATION FAILED:', otpCreated);
        throw errorUtilities.createError(OtpResponses.OTP_CREATION_FAILED, StatusCodes.InternalServerError);
    }
    const emailData = {
        email: user.email,
        otp: otp,
    ...
    try {
        await axios.post(`${config.NOTIFICATION_SERVICE_ROUTE}/auth-notification/resend-verification-otp`, emailData);
    } catch (error: any) {
        console.error(`Error Resending Verification Mail: ${error.message}`);
        throw errorUtilities.createError(OtpResponses.OTP_RESEND_FAILED, StatusCodes.InternalServerError);
    }

    return responseUtilities.handleServicesResponse(StatusCodes.Created, GeneralResponses.USER_REGSTRATION_SUCCESSFUL, user.email)

})


const loginUserService = errorUtilities.withServiceErrorHandling(async (loginPayload: { email: string, password: string, stayLoggedIn: boolean }) => {

    const { email, password, stayLoggedIn } = loginPayload;

    const user = await usersRepositories.getOne({ email }, ['id', 'email', 'password', 'role', 'isActive', 'isBlocked', 'isVerified', 'firstName', 'lastName', 'isFirstTimeLogin']);
    ...
    if (!user.isActive) {
        throw errorUtilities.createError(GeneralResponses.INACTIVE_ACCOUNT, StatusCodes.Forbidden);
    }

    if (user.isBlocked) {
        throw errorUtilities.createError(GeneralResponses.BLOCKED_ACCOUNT, StatusCodes.Forbidden);
    }

    if (!user.isVerified) {
        throw errorUtilities.createError(GeneralResponses.UNVERIFIED_ACCOUNT, StatusCodes.Forbidden);
    }

    const isPasswordValid = await helperFunctions.comparePasswords(password, user.password);

    if (!isPasswordValid) {
        throw errorUtilities.createError(GeneralResponses.INVALID_CREDENTIALS, StatusCodes.Unauthorized);
    }
    ...
    return responseUtilities.handleServicesResponse(StatusCodes.OK, GeneralResponses.SUCCESSFUL_LOGIN, { token, user: userDetails });
})


const passwordResetRequestService = errorUtilities.withServiceErrorHandling(async (email: string) => {
    const user = await usersRepositories.getOne(
        { email: email },
        ['id', 'email', 'role', 'isVerified', 'firstName', 'isActive', 'isBlocked']
    );
    if (!user) {
        throw errorUtilities.createError(GeneralResponses.USER_NOT_FOUND, StatusCodes.NotFound);
    }
    ...
    const emailData = {
        email: user.email,
        resetUrl: `${config.PROD_PASSWORD_RESET_URL!}?token=${token}`,
        firstName: user.firstName
    }
    try {
